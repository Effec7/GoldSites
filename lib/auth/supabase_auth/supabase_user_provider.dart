...
class GoldSitesSupabaseUser extends BaseAuthUser {
  GoldSitesSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> goldSitesSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = GoldSitesSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
